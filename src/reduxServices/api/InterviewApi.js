export const API_URL="http://localhost:8000";



export const INTERVIEW_API = API_URL + "/questions/"; 
export const CANDIDATE_API = API_URL + "/Candidate/"; 
export const ANSWER_API = API_URL + "/answer/"; 
export const HR_API = API_URL + "/hr/"; 
export const PHOTO_API = API_URL + "/photo/"; 
export const REQUIREMENT_API = API_URL + "/requirement/";
export const REGISTER_API = API_URL + "/register/";
export const CHAT_API = API_URL + "/chat/";
export const JD_ASSISTANT_API = API_URL + "/jd-assistant/"; 
export const AI_QUETION_API = API_URL + "/Ai-question/"; 
export const QUETION_BANK_API = API_URL + "/questions-bank/"; 


